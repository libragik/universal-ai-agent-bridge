...
8. **Dynamic Response Caching & Semantic Cache Engine (`cache.mjs`, `llm_cache`, `agy-llm cache`)**: Instant 0ms cache hits for identical queries using deterministic SHA-256 canonical hashing, configurable TTL expiration, and automatic LRU eviction. Slashes token expenditure to zero on repetitive tasks, tests, and code reviews, with live hit/miss and USD savings telemetry.
9. **Zero-Dependency Core**: Pure Node.js ESM runtime using native `fetch` and high-speed stdio streams. Boots in ~15ms with zero npm installation overhead.
10. **Multi-Provider Vault**: Store any number of named provider profiles with custom base URLs, tokens, default models, and custom headers.
11. **DeepSeek-R1 / MiniMax Reasoning Extraction**: Automatically parses `<think>` tags and `reasoning_content` properties so reasoning models never break schemas.
12. **Ad-Hoc Endpoint Overrides**: Query any temporary `/v1` endpoint on the fly simply by passing `endpoint_url` and `api_key` in natural language.
...
14. **Cross-Model Comparison & Benchmarking**: Parallel query runner (`llm_compare`) to benchmark code solutions side-by-side across multiple models.
15. **1-Click Portable Installers**: Seamless setup scripts for Windows, macOS, and Linux to clone and use on any PC.
